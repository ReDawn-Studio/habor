...
- 鼠标拖选输出显示高亮，松开自动复制；右键 / Ctrl+C 再次复制，Esc 先取消选区。
- 保留内部滚轮和拖动边缘滚动；选区固定显示内容，避免流式更新改变正在复制的文字。支持中文、组合字符、emoji 和跨行选择。
...
- 官方历史项显示来源，恢复时导入原始用户与助手消息到当前模型上下文。
- 只读扫描官方历史文件 / 索引，不修改官方会话数据；仍按当前工作目录隔离。
