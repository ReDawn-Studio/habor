...
import { agentHealthPlan } from "./agent-health.js";
...
export class AgentInstaller {
  ...
  async install(id: string, options: RunOptions = {}): Promise<string> {
    ...
    try {
      ...
      const result = await this.runner({ command: npm, args: ["install", "--prefix", stage, "--cache", cache, "--no-audit", "--no-fund", "--package-lock=true", "--registry=https://registry.npmjs.org", ...plan.flags, `${plan.package}@${plan.version}`], cwd: stage }, { ...options, timeoutMs: options.timeoutMs ?? 600000 });
      ...
      const healthDir = join(stage, ".habor-health");
      const health = agentHealthPlan(id, binary, healthDir);
      if (health) {
        mkdirSync(healthDir, { recursive: true, mode: 0o700 });
        options.onOutput?.("正在检查原生客户端启动与协议握手（不调用模型）…");
        try {
          const result = await this.runner(health, { signal: options.signal, onOutput: options.onOutput, timeoutMs: 30000 });
          if (result.code !== 0) throw new Error(`客户端启动检查失败，尚未启用此版本；原有客户端保持可用。${result.output.slice(-3000)}`);
        } finally { rmSync(healthDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); }
      }
      if (options.signal?.aborted) throw new Error("操作已取消");
      const temporary = join(root, `.active-${install}.json`);
      writeFileSync(temporary, JSON.stringify({ install, package: plan.package, version: manifest.version, installedAt: new Date().toISOString(), checks: health ? ["version", "protocol-initialize"] : ["version"] }), { mode: 0o600, flag: "wx" });
